...
public class M2C {
	...
	try {
		...
		for(int i = 2; i<Array.getLength(args); i++) {
	    ...
	    args[1] = args[1].replaceAll("[^a-zA-Z0-9]TXT$", "");
	    if(ucsc) {
		if(!(args[1].endsWith("cov"))) {
		    args[1] = args[1] + ".cov";
		}
		logFileName = args[1].replaceAll("[^a-zA-Z0-9]cov$", ".log");
	    }
	    else {
		if(!(args[1].endsWith("cov"))) {
		    args[1] = args[1] + ".cov";
