...
if(@ARGV < 1) {
    die "
Usage: sort_file_by_loc.pl <file> [<chr col> <start col> <end col>] [options]

<file> should have the three columns: chr, start, end
default is chr=col1, start=col2, end=col3.  Otherwise
specify which columns, as the optional args (start
counting at 1).

options:
           -s n  : skip n lines at the beginning, just echo them back

";
}

if($ARGV[1] =~ /^\d+$/ && $ARGV[2] =~ /^\d+$/ && $ARGV[3] =~ /^\d+$/) {
    $chrcol = $ARGV[1]-1;
    $startcol = $ARGV[2]-1;
    $endcol = $ARGV[3]-1;
} else {
