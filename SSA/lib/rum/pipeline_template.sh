...
echo "finished first BLAT run" `date` `date +%s` >> OUTDIR/rum.log_chunk.CHUNK
...
echo "finished parsing first BLAT run" `date` `date +%s` >> OUTDIR/rum.log_chunk.CHUNK
...
perl SCRIPTSDIR/sort_RUM_by_id.pl OUTDIR/RUM_NU_temp2.CHUNK OUTDIR/RUM_NU.CHUNK
echo "finished sorting final results" `date` `date +%s` >> OUTDIR/rum.log_chunk.CHUNK
