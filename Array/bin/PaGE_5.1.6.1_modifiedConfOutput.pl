...
# NOTE: this version hacked for Elisabetta to output gene_confidence_list in another format
...
if($output_gene_conf_list == 1) {
    ...
    if($design eq "D") {
	print GENECONFLISTOUTFILE "id";
	for(my $i=$start; $i<$num_conds; $i++) {
	    print GENECONFLISTOUTFILE "\tconf $i up\tconf $i down\tmean ratio $i";
	}
	print GENECONFLISTOUTFILE "\n";
	foreach my $id (keys %gene_confidences_up) {
	    print GENECONFLISTOUTFILE "$id";
	    for(my $i=$start; $i<$num_conds; $i++) {
		my $xx = $gene_confidences_up{$id}[$i];
		print GENECONFLISTOUTFILE "\t$xx";
		$xx = $gene_confidences_down{$id}[$i];
		print GENECONFLISTOUTFILE "\t$xx";
 		$xx = ComputeMean(\@{$data[$ids_hash{$id}][$i]},$missing_value_designator,1);
		print GENECONFLISTOUTFILE "\t$xx";
	    }
	    print GENECONFLISTOUTFILE "\n";
	}
	...
	print GENECONFLISTOUTFILE "id";
	for(my $i=$start; $i<$num_conds; $i++) {
	    print GENECONFLISTOUTFILE "\tconf $i up\tconf $i down\tmean 0\tmean $i";
	}
	print GENECONFLISTOUTFILE "\n";
	foreach my $id (keys %gene_confidences_up) {
	    print GENECONFLISTOUTFILE "$id";
		...
		$xx = $gene_confidences_down{$id}[$i];
		print GENECONFLISTOUTFILE "\t$xx";
		$xx = $unpermuted_means[$ids_hash{$id}][0];
		print GENECONFLISTOUTFILE "\t$xx";
		$xx = $unpermuted_means[$ids_hash{$id}][$i];
		print GENECONFLISTOUTFILE "\t$xx";
	    }
	    print GENECONFLISTOUTFILE "\n";
	}
